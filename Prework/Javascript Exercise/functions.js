...
function dniLetter( dni ) {
	...
	var truenum=Math.abs(Number(dni));
...
function findLongestWord( array_words ){
	...
	for(var i=0; i<array_words.length; i++){
		var wordlength=array_words[i].length;

		if(longest<array_words[i].length){
			longest = wordlength;
		}
	}
	return longest;
...
array_lengths.push( findLongestWord( ["What", "Is", "Going", "On"] ) );
...
function calculateAverage( array )
{
	...
	var avg = sum/array_lengths.length;
	return avg;
}
